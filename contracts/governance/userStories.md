...
## **Context**
For context, please see `specs.md`. 
...
The ‘Blank Canvas Forum’ Component is a standardised dynamic website that DAOs can clone and customise to make their own DAO Portal. 
This ‘blank canvas forum’ is a front-end website build on XMTP. It has two main features: 
- Voting - on mandates.
- Forum / Chatting - with other members inside the DAO’s ecosystem.

The forum structures threads along mandates and governance flows: grouping mandates together that are linked together in governance flows. Each governance flow has a combined chatroom where all roles of that governance flow can chat together; each single mandate with a vote has its own dedicated chatroom, and each vote gets its own thread - while the vote is active. 
